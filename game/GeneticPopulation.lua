...
GeneticPopulation.update = Population.update
GeneticPopulation.draw = Population.draw
GeneticPopulation.get_population = Population.get_population
...
function GeneticPopulation:add_to_history(this)
	...
	if #self._history == self._population_size then
		local lowest, lowest_index = utils.get_lowest(self._history, "_fitness")
...
function GeneticPopulation:replace(i)
	...
	if self._random_init > 0 then
		...
		-- cross
		self._population[i]:crossover(mom, dad, self._reset_table)
	end
end
