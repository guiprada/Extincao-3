...
function Ghost:reset(	pos_index,
						...
						grid_pos,
						direction)
	local valid_grid_pos = Ghost._grid.valid_pos[pos_index]
	self.pos_index = pos_index
	self.home.x = valid_grid_pos.x
	self.home.y = valid_grid_pos.y

	local grid_pos = grid_pos or self.home
...
function Ghost:crossover(ghosts, spawn_grid_pos)
	...
	son.target_offset = random.random(math.floor(-Ghost._target_spread), math.ceil(Ghost._target_spread))
...
function Ghost.selection(in_table)
	...
	local best_stack = utils.get_n_best(in_table, "fitness", math.floor(#in_table/10))
	mom = best_stack[random.random(1, #best_stack)]
	dad = in_table[random.random(1, #in_table)]
...
function Ghost:update(targets, pills, average_ghost_pos, dt)
	if (self.is_active) then
		...
		local dist_grid_center = utils.distance( Ghost._grid:get_grid_center(self.grid_pos), self)
...
function Ghost:wander(possible_next_moves)
	local destination = {}
	local rand_grid = random.random(1, #Ghost._grid.valid_pos )
...
function Ghost:get_closest(possible_next_moves, destination)
	local shortest = 1
	local shortest_distance = utils.distance(possible_next_moves[1], destination)
	for i = 2, #possible_next_moves, 1 do
		local dist = utils.distance(possible_next_moves[i], destination)
		if (dist < shortest_distance) then
			shortest = i
			shortest_distance = dist
		end
	end
	self.direction = possible_next_moves[shortest].direction
end

function Ghost:get_furthest(possible_next_moves, destination)
	local furthest = 1
	for i=1, #possible_next_moves, 1 do
		possible_next_moves[i].dist = utils.distance(possible_next_moves[i], destination)
		if ( possible_next_moves[i].dist > possible_next_moves[furthest].dist ) then
			furthest = i
		end
	end
	--print("furthest" .. furthest)
	self.direction = possible_next_moves[furthest].direction
end
