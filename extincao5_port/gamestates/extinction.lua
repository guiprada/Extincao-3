...
local population_save = require "entities.population_save"
	...
	if not gs.game_conf then
		...
		if gs.game_conf.autoplayer_neat_enable then
			AutoPlayer_NEAT.init(
				...
				gs.game_conf.autoplayer_neat_loopback_link_chance,
		...
		gs.actions_keyup['p'] =
			function ()
				if population_save.save(gs.GhostPopulation) then
					print("population saved")
				end
			end
		gs.actions_keyup['l'] =
			function ()
				if population_save.load(gs.GhostPopulation) then
